...
from loan.models import LoanApplication as loanapplication,GuarantorRequest,LoanAmortizationSchedule,LoanRepayment as loanrepayment,LoanGuarantor, LockedShares
                    ...
                    if request.user.check_password(pin):
                        ...
                        if valid:
                            ...
                            try:
                                ...
                                message = "Your loan of {} {} has been successfully processed.Your wallet has been credited with {} {}".format(member.currency, loan_amount, member.currency, loan_amount)
                ...
                if not loan.is_fully_repaid:
                    ...
                    if valid:
                        ...
                        if repayment_amount > latest_loan_amortize.total_repayment and remaining_number_of_months == 0:
                            msg = "The amount entered exceeds this month's loan repayment amount of {} {}".format(member.currency, latest_loan_amortize.total_repayment)
                        ...
                        try:
                            ...
                            if remaining_number_of_months == 0 or ending_balance == 0:
                                ...
                                shares_desc = "{} confirmed.Shares worth {} {} that were locked to guarantee your loan {} of {} {} have been unlocked.".format(shares_transaction_code, member.currency, unlocked_num_of_shares, loan.loan_code, member.currency, loan.amount)
                ...
                if guarantor.has_accepted is None:
                    ...
                    if has_accepted:
                        ...
                        if circle_instance.get_available_circle_member_shares(circle, member) >= guarantor.num_of_shares:
                            ...
                            shares = circle_member.shares.get()
                            shares_transaction_code = general_instance.generate_unique_identifier('STL')
                            shares_desc = "{} confirmed.Shares worth {} {} have been locked to guarantee loan {} of {} {}.".format(shares_transaction_code, member.currency, guarantor.num_of_shares, loan.loan_code, loan_member.user.first_name, loan_member.user.last_name)
                        ...
                        fcm_data = {"request_type":"UPDATE_GUARANTOR_REQUEST_STATUS",
                                    "loan_code":loan.loan_code,"circle_acc_number":circle.circle_acc_number,
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                shares_transaction_code = general_instance.generate_unique_identifier('STU')
                shares_desc = "{} confirmed.Shares worth {} {} that were locked to guarantee your loan {} of {} {} have been unlocked following cancellation of loan.".format(shares_transaction_code, member.currency, num_of_shares, loan.loan_code, member.currency, amount)
